...
def cleanup_old_reports(keep_count: int = 5, dry_run: bool = False) -> None:
    ...
    for report in reports:
        ...
        if match:
            ...
            try:
                ...
                report_dates.append((date_obj, report))
            except ValueError:
    ...
    print(f"📊 {len(report_dates)} rapports automatiques trouvés")
    
    if len(report_dates) <= keep_count:
        print(f"✅ Aucun nettoyage nécessaire (≤ {keep_count} rapports)")
        return
    
    # Identification des rapports à supprimer
    to_keep = report_dates[:keep_count]
    to_delete = report_dates[keep_count:]
    
    print(f"\n📋 Rapports à conserver ({keep_count}) :")
    for date_obj, report in to_keep:
        size = os.path.getsize(report) / 1024  # KB
        print(f"  ✅ {report} ({date_obj.strftime('%d/%m/%Y')}, {size:.1f} KB)")
    
    print(f"\n🗑️  Rapports à supprimer ({len(to_delete)}) :")
    for date_obj, report in to_delete:
        size = os.path.getsize(report) / 1024  # KB
        print(f"  ❌ {report} ({date_obj.strftime('%d/%m/%Y')}, {size:.1f} KB)")
    ...
    if to_delete:
        response = input(f"\n❓ Confirmer la suppression de {len(to_delete)} rapports ? (y/N): ")
