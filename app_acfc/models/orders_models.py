...
from app_acfc.db_models.orders import Order, DevisesFactures
...
class OrdersModel:
    """
    Modèle pour la gestion des commandes.
    Fournit des méthodes pour créer, mettre à jour, annuler et récupérer les commandes.
    """
    def __init__(self, request: Request) -> None:
        ...
        self.is_new: bool
        self.order_details: Optional[Order]
        ...
        self.invoiced: bool
        self.billed: bool
        self.part_billed: bool
        self.order: Order

    def __check_order_status(self, *, id_order: Optional[int], id_client: int,
        ...
        if id_order:
            ...
            self.serveur_entries: List[DevisesFactures] = self.db_session.query(DevisesFactures) \
                                                                .filter_by(id_order=id_order).all()
    ...
    def __check_entries_status(self, *, id_order: int, id_client: int) -> None:
        """
    ...
    def _complete_create_or_update(self, *, entry: DevisesFactures) -> DevisesFactures:
        ...
        """
        if self.is_new:
            entry.created_by = session.get('pseudo', 'system')
        else:
    ...
    def _canceled_entries_filter(self) -> None:
        ...
        if self.order_details:
            self.order_details.devises = [
                entry for entry in self.order_details.devises if not entry.is_annulee
                ]
        else:
    ...
    def post_order_data(self, *, id_client: int, id_order: Optional[int]) -> 'OrdersModel':
        ...
        """
        self.__check_order_status(id_order=id_order, id_client=id_client)

        # Gestion de la commande
        self.order.id_client = id_client
        self.order.date_commande = datetime.strptime(self.request.form.get('date_commande', ''),
                                                     '%Y-%m-%d').date()
        self.order.id_adresse_facturation = self.request.form.get('id_adresse_facturation', None)
        self.order.id_adresse_livraison = self.request.form.get('id_adresse_livraison',
                                                                self.order.id_adresse_facturation)
        self.order.descriptif = self.request.form.get('descriptif', '').strip()
        self.order.is_ad_livraison = (self.order.id_adresse_livraison !=
                                      self.order.id_adresse_facturation)
            ...
            if key.startswith('lignes_'):
                ...
                if raw_json:
                    try:
                        ...
                        _id = self._is_new_line(entry_id=product.get('id', None),
                                                is_new_order=self.is_new)
        ...
        sum_server_value = round(float(
            sum(entry.prix_unitaire * entry.qte * (1 - entry.remise) \
        ...
        if abs(sum_client_value - sum_server_value) > 0.01:
            ...
            message = f'Erreur importante de montant détectée pour la commande ID {self.order.id}' \
                 + f' du client ID {id_client} : {sum_client_value} != {sum_server_value}'
            acfc_log.log(level=WARNING, message=message, db_log=True)
    ...
    def check_entries_changements(self) -> 'OrdersModel':
        """
        Compare les entrées du client avec celles du serveur pour déterminer les ajouts,
        ...
        entries_to_add: List[DevisesFactures] = [entry for entry in self.client_entries
                                                 if entry.id is None]

        # Mettre à jour les produits existants
        entries_to_update: List[DevisesFactures] = [entry for entry in self.client_entries
                                                    if entry.id in serveur_dict]

        # Supprimer les produits retirés
        entries_to_delete: List[DevisesFactures] = [entry for entry in self.serveur_entries
                                                    if entry.id not in client_entries]
    ...
    def cancel_order(self, *, id_client: int, id_order: int) -> 'OrdersModel':
        ...
        self.__check_order_status(id_order=id_order, id_client=id_client)
        self.__check_entries_status(id_order=id_order, id_client=id_client)
        if self.billed or self.part_billed:
            raise ValueError("Impossible d'annuler une commande facturée même partiellement.")
        else:
